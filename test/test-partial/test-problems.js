...
describe('Problems partial matching library (problems.js) tests', function() {
        ...
        it('compare two different problems sections', function() {
            ...
            expect(_.where(m, {dest: 'dest'}).length).to.equal(4);
            expect(_.where(m, {dest: 'src'}).length).to.equal(2);
        ...
        it('compare problems sections with itself', function() {
            ...
            for (var objArray in src_obj_array) {
                ...
                expect(_.where(src_obj_array[objArray], {dest: 'dest'}).length).to.equal(2);
                expect(_.where(src_obj_array[objArray], {dest: 'src'}).length).to.equal(1);
                expect(_.where(src_obj_array[objArray], {dest: 'dest', match: 'duplicate'}).length).to.equal(1);
                expect(_.where(src_obj_array[objArray], {dest: 'dest', match: 'new'}).length).to.equal(1);
                expect(_.where(src_obj_array[objArray], {dest: 'src', match: 'duplicate'}).length).to.equal(0);
                expect(_.where(src_obj_array[objArray], {dest: 'src', match: 'new'}).length).to.equal(1);
            }
        ...
        it('compare two different problems sections that will have some partial match, some dups and some new', function() {
            ...
            //Match One.
            expect(_.where(src_obj_array[0], {dest: 'dest'}).length).to.equal(4);
            expect(_.where(src_obj_array[0], {dest: 'src'}).length).to.equal(1);
            expect(_.where(src_obj_array[0], {dest: 'dest', match: 'new'}).length).to.equal(2);
            expect(_.where(src_obj_array[0], {dest: 'src', match: 'new'}).length).to.equal(1);
            ...
            //Match Two.
            expect(_.where(src_obj_array[1], {dest: 'dest'}).length).to.equal(4);
            expect(_.where(src_obj_array[1], {dest: 'src'}).length).to.equal(1);
            expect(_.where(src_obj_array[1], {dest: 'dest', match: 'new'}).length).to.equal(3);
            expect(_.where(src_obj_array[1], {dest: 'src', match: 'new'}).length).to.equal(1);
            expect(_.where(src_obj_array[1], {dest: 'dest', match: 'partial'}).length).to.equal(0);
            expect(_.where(src_obj_array[1], {dest: 'dest', match: 'duplicate'}).length).to.equal(1);
