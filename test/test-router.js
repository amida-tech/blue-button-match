...
describe('Matching library (compare-partial.js) tests', function () {
  ...
  describe('Exceptions test', function () {
    it('testing exceptions', function () {
      ...
      expect(fn).toThrowError('one argument is required for compare function');
    });
  });
        ...
        if (bb.hasOwnProperty(name)) {
          for (var entry in bb[name]) {
            ...
            if (name !== 'demographics') {
              expect(compare(bb[name][entry], bb[name][entry], name)).toHaveProperty("match", "duplicate");
            }
          }
        }
      }
      ...
      for (var section in lookups.sections) {
        ...
        if (bb.hasOwnProperty(name)) {
          for (var entry in bb2[name]) {
            //console.log(bb.data[name][entry]);
            if (name !== 'demographics') {
              expect(compare(bb2[name][entry], bb2[name][entry], name)).toHaveProperty("match", "duplicate");
            }
          }
        }
      }
      ...
      for (var section in lookups.sections) {
        ...
        if (bb.hasOwnProperty(name)) {
          for (var entry in bb3[name]) {
            //console.log(bb.data[name][entry]);
            if (name !== 'demographics') {
              expect(compare(bb3[name][entry], bb3[name][entry], name)).toHaveProperty("match", "duplicate");
            }
          }
        }
      }
      ...
      for (var section in lookups.sections) {
        ...
        if (bb4.hasOwnProperty(name)) {
          for (var entry in bb4[name]) {
            ...
            if (name !== 'demographics') {
              expect(compare(bb4[name][entry], bb4[name][entry], name)).toHaveProperty("match", "duplicate");
            }
          }
        }
      }
