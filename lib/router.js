...
exports.compare = function compare(a, b, section) {
    ...
    if (equal(a, b)) {
        ...
        var calculateMatch = function (primaryEntries, secondaryEntries, object_a, object_b) {
            ...
            for (var i in primaryStringArrayEntries) {

                var matchedArrays = false;
                
                var inputObjectA = object_a[primaryStringArrayEntries[i].path];
                var inputObjectB = object_b[primaryStringArrayEntries[i].path];
                ...
                if (matchedArrays) {
                    matchPercent = matchPercent + primaryStringArrayEntries[i].percent;
                }
            ...
            for (var i in primaryStringEntries) {
                ...
                if (primaryStringEntries[i].path.indexOf(".") > -1) {
                    ...
                    for (var splitEntry in strSplit) {
                        if (inputObjectA[strSplit[splitEntry]] !== undefined && inputObjectB[strSplit[splitEntry]] !== undefined) {
                    ...
                    inputObjectA = object_a[primaryStringEntries[i].path];
                    inputObjectB = object_b[primaryStringEntries[i].path];
                }
                ...
                if (utils.stringMatch(inputObjectA, inputObjectB)) {
            ...
            for (var i in primaryObjectEntries) {

                if (utils.objectMatch(object_a[primaryObjectEntries[i].path], object_b[primaryObjectEntries[i].path])) {
                    console.log(matchPercent);
                    matchPercent = matchPercent + primaryObjectEntries[i].percent;
                }
